...
TEST(S21VectorTest, InsertIterators) {
  // Test insert operation using iterators for s21::vector
  s21::vector<int> s21v{1, 2, 3};
  s21::vector<int>::iterator it = s21v.insert(s21v.begin() + 1, 4);
  EXPECT_EQ(*it, 4);
  EXPECT_EQ(s21v.size(), 4);
  EXPECT_EQ(s21v[1], 4);

  // Test insert operation using iterators for std::vector
  std::vector<int> stdv{1, 2, 3};
  std::vector<int>::iterator std_it = stdv.insert(stdv.begin() + 1, 4);
  EXPECT_EQ(*std_it, 4);
  EXPECT_EQ(stdv.size(), 4);
  EXPECT_EQ(stdv[1], 4);

  // Compare the results of s21::vector and std::vector
  for (int i = 0; i < s21v.size(); ++i) {
    EXPECT_EQ(s21v[i], stdv[i]);
  }
}

TEST(S21VectorTest, InsertCountValue) {
  // Test insert operation using count and value for s21::vector
  s21::vector<int> s21v{1, 2, 3};
  s21v.insert(s21v.begin() + 1, 4);
  EXPECT_EQ(s21v.size(), 4);
  EXPECT_EQ(s21v[1], 4);

  // Test insert operation using count and value for std::vector
  std::vector<int> stdv{1, 2, 3};
  stdv.insert(stdv.begin() + 1, 4);
  EXPECT_EQ(stdv.size(), 4);
  EXPECT_EQ(stdv[1], 4);
...
TEST(S21VectorTest, InsertCountValueConst) {
  // Test insert operation using count and value for s21::vector
  const int &insert = 4;
  s21::vector<int> s21v{1, 2, 3};
  s21v.insert(s21v.begin() + 1, insert);
  EXPECT_EQ(s21v.size(), 4);
  EXPECT_EQ(s21v[1], 4);

  // Test insert operation using count and value for std::vector
  std::vector<int> stdv{1, 2, 3};
  stdv.insert(stdv.begin() + 1, insert);
  EXPECT_EQ(stdv.size(), 4);
  EXPECT_EQ(stdv[1], 4);
...
// Test if s21::vector behaves like std::vector when it comes to size and
// capacity
